...
class TestSessionSecurity(unittest.TestCase):
    ...
    def test_session_cookie_settings(self):
        ...
        with self.app as client:
            ...
            cookie_header = client.cookie_jar._cookies.get('localhost.local', {}).get('/', {}).get('session')
            if cookie_header:
...
class TestCSRFProtection(unittest.TestCase):
    ...
    def test_csrf_token_present(self):
        ...
        csrf_token_match = re.search(r'<input[^>]*name="csrf_token"[^>]*value="([^"]*)"', response.data.decode())
        self.assertIsNotNone(csrf_token_match, "CSRF token not found in login form")
        
        # Also check registration form
        response = self.app.get('/register')
        csrf_token_match = re.search(r'<input[^>]*name="csrf_token"[^>]*value="([^"]*)"', response.data.decode())
        self.assertIsNotNone(csrf_token_match, "CSRF token not found in registration form")
