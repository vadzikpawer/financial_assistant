...
def generate_ai_recommendations(user_id):
    ...
    try:
        ...
        current_month_expenses = df[
            (df["is_expense"] == True) & (df["date"] >= current_month)
        ]
        prev_month_expenses = df[
            (df["is_expense"] == True)
            & (df["date"] >= prev_month)
        ...
        for rec in ai_recommendations:
            ...
            for cat_id, cat_obj in categories.items():
                if (
                    cat_obj.name.lower() in rec["title"].lower()
...
def get_recommendations_from_aiml(financial_data):
    ...
    if not ai_api_active or not aiml_client:
        logger.warning(
            "AIML API is not active or not configured. Using rule-based recommendations."
        )
    ...
    try:
        ...
        logger.error(
            f"Error getting recommendations from AIML API: {str(e)}", exc_info=True
        )
...
def get_rule_based_recommendations(
    financial_data, quota_exceeded=False, api_error=False
):
    """
    ...
    if financial_data and "financial_stats" in financial_data:
        ...
        if "current_month_spent" in stats and "prev_month_spent" in stats:
            ...
            if (
                previous > 0 and current > previous * 1.15
            ):  # Spending increased by more than 15%
                recommendations.append(
                    {
