...
import pandas as pd
import requests
from deeperseek import DeeperSeek  # Import DeeperSeek library
...
# Initialize DeeperSeek client
if DEEPSEEK_API_KEY:
    try:
        # Initialize DeeperSeek client with the API key
        deepseek_client = DeeperSeek(api_key=DEEPSEEK_API_KEY)
        logger.info("DeeperSeek client initialized successfully")
    except Exception as e:
        logger.error(f"Failed to initialize DeeperSeek client: {str(e)}")
        ai_api_active = False
else:
    logger.warning(
        "No DeepSeek API key found. AI recommendations will be disabled.")
    ai_api_active = False
...
def generate_ai_recommendations(user_id):
    ...
    try:
        ...
        old_recommendations = Recommendation.query.filter_by(
            user_id=user_id).all()
        ...
        transactions = (Transaction.query.join(BankAccount).filter(
            BankAccount.user_id == user_id).all())
        ...
        if not transactions:
            ...
            new_recommendation = Recommendation(
                ...
                description=
                "Подключите банковские счета, чтобы получать детальный анализ ваших расходов и персонализированные рекомендации по экономии.",
        ...
        transactions_data = [{
            ...
            if t.category_id and t.category_id in categories else "Unknown",
            'is_expense':
            t.is_expense,
            'account_id':
            t.account_id
        ...
        # Overall spending
        stats["total_spent"] = float(
            df[df['is_expense'] == True]['amount'].sum())
        stats["total_income"] = float(
            df[df['is_expense'] == False]['amount'].sum())
        ...
        current_month_expenses = df[(df['is_expense'] == True)
                                    & (df['date'] >= current_month)]
        prev_month_expenses = df[(df['is_expense'] == True)
                                 & (df['date'] >= prev_month) &
                                 (df['date'] < current_month)]

        stats["current_month_spent"] = float(
            current_month_expenses['amount'].sum(
            ) if not current_month_expenses.empty else 0)
        stats["prev_month_spent"] = float(prev_month_expenses['amount'].sum(
        ) if not prev_month_expenses.empty else 0)
        ...
        if ai_api_active and deepseek_client:
            ...
            logger.info(
                "Using rule-based recommendations (DeepSeek API not available)"
            )
            using_ai = False
        ...
        for rec in ai_recommendations:
            ...
            for cat_id, cat_obj in categories.items():
                if cat_obj.name.lower() in rec["title"].lower(
                ) or cat_obj.name.lower() in rec["description"].lower():
                    category_id = cat_id
                    break

            # Create recommendation
            new_recommendation = Recommendation(user_id=user_id,
                                                title=rec["title"],
        ...
        try:
            fallback_rec = Recommendation(
                ...
                description=
                "Регулярно анализируйте свои расходы, сократите ненужные подписки и старайтесь готовить дома чаще. Эти простые шаги могут помочь вам сэкономить до 15% ваших ежемесячных расходов.",
                potential_savings=0.0)
...
def get_recommendations_from_deepseek(financial_data):
    ...
    if not ai_api_active or not deepseek_client:
        logger.warning(
            "DeepSeek API is not active or not configured. Using rule-based recommendations."
        )
    ...
    try:
        ...
        # Call DeeperSeek API - it has a simpler interface than DeepSeek
        ...
        logger.error(f"Error getting recommendations from DeepSeek: {str(e)}",
                     exc_info=True)
        # Return rule-based recommendations
        return get_rule_based_recommendations(financial_data)
...
def get_rule_based_recommendations(financial_data,
    ...
    if quota_exceeded:
        recommendations.append({
            ...
            "description":
            "В данный момент сервис AI-рекомендаций временно недоступен из-за превышения лимита API запросов. Мы предоставляем базовые рекомендации по экономии. Пожалуйста, попробуйте обновить рекомендации позже.",
            "potential_savings": 0.0
        })
    elif api_error:
        recommendations.append({
            "title": "Уведомление о сервисе рекомендаций",
            "description":
            "В данный момент сервис AI-рекомендаций временно недоступен из-за технических проблем. Мы предоставляем базовые рекомендации по экономии. Пожалуйста, попробуйте обновить рекомендации позже.",
            "potential_savings": 0.0
        })
    ...
    basic_recommendations = [{
        ...
        "description":
        "Проанализируйте регулярные платежи и подписки. Отмените сервисы, которыми вы редко пользуетесь, и рассмотрите возможность перехода на более выгодные тарифы для необходимых услуг.",
        "potential_savings": 1000.0
    }, {
        "title": "Составьте бюджет на продукты",
        "description":
        "Планируйте покупки продуктов заранее, составляйте списки и не ходите в магазин голодными. Это поможет избежать импульсивных покупок и сэкономить на продуктах питания.",
        "potential_savings": 2000.0
    }, {
        "title": "Сократите расходы на доставку еды",
        "description":
        "Доставка еды обычно стоит на 30-50% дороже, чем приготовление пищи дома. Попробуйте готовить большие порции на несколько дней вперед, чтобы сэкономить время и деньги.",
        "potential_savings": 3000.0
    }, {
        "title": "Откладывайте 10% дохода",
        "description":
        "Попробуйте автоматически откладывать 10% от всех доходов в день получения зарплаты. Создайте отдельный сберегательный счет для этой цели и настройте автоматические переводы.",
        "potential_savings": 5000.0
    }, {
        "title": "Используйте кэшбэк и бонусные программы",
        "description":
        "Подключите банковские карты с кэшбэком и используйте программы лояльности в магазинах, где вы регулярно совершаете покупки. Это позволит вам вернуть часть ваших расходов.",
    ...
    if financial_data and "financial_stats" in financial_data:
        ...
        if "category_spending" in stats and stats["category_spending"]:
            ...
            if sorted_categories:
                ...
                for idx, (category,
                          amount) in enumerate(sorted_categories[:2]):
                    ...
                    if idx == 0:  # First category (highest spending)
                        recommendations.append({
                            "title":
                            f"Сократите расходы на категорию '{category}'",
                            "description":
                            f"Это ваша самая большая категория расходов - {int(amount)} ₽. Рассмотрите способы сократить эти траты на 15-20%, например, сравнивайте цены перед покупкой или ищите альтернативные решения.",
                            "potential_savings":
                            potential_savings
                        })
                        ...
                        recommendations.append({
                            "title":
                            f"Оптимизируйте расходы в категории '{category}'",
                            "description":
                            f"Вы тратите значительную сумму ({int(amount)} ₽) на '{category}'. Подумайте, как можно оптимизировать эти расходы, например, искать скидки, акции или более выгодные предложения.",
                            "potential_savings":
                            potential_savings
                        })
        ...
        if "current_month_spent" in stats and "prev_month_spent" in stats:
            ...
            if previous > 0 and current > previous * 1.15:  # Spending increased by more than 15%
                recommendations.append({
                    "title":
                    "Контролируйте рост расходов",
                    "description":
                    f"В текущем месяце ваши расходы ({int(current)} ₽) превышают расходы предыдущего месяца ({int(previous)} ₽) на {int((current/previous - 1) * 100)}%. Проанализируйте, в каких категориях выросли расходы, и попробуйте их сократить.",
                    "potential_savings":
                    round((current - previous) * 0.5)  # 50% of the increase
                })
