...
def init_categories():
    ...
    try:
        ...
        if category_count == 0:
            ...
            db.session.commit()
            logger.info(
                f"Initialized {len(default_categories)} categories from scratch"
            )
            return True
        else:
...
def analyze_transactions(transactions_df):
    ...
    try:
        ...
        prev_month_expenses = expenses_df[
            (expenses_df["date"] >= start_of_prev_month)
            & (expenses_df["date"] < start_of_month)
        ]
        prev_month_spent = (
            prev_month_expenses["amount"].sum() if not prev_month_expenses.empty else 0
        )
        ...
        if prev_month_spent > 0:
            month_over_month_change = (
                (current_month_spent - prev_month_spent) / prev_month_spent
            ) * 100

        # Spending by category (current month)
        category_spending = (
            current_month_expenses.groupby("category_name")["amount"]
            .sum()
            .reset_index()
        )
        ...
        day_spending["day_order"] = day_spending["day_of_week"].map(
        ...
        daily_avg = (
            total_spent / days_with_transactions if days_with_transactions > 0 else 0
        )
        ...
        if std_amount > 0:
            recent_expenses = expenses_df[
                expenses_df["date"] >= (today - timedelta(days=30))
            ]
            recent_expenses["z_score"] = (
                recent_expenses["amount"] - mean_amount
            ) / std_amount
            unusual_transactions = recent_expenses[
                recent_expenses["z_score"] > 2
            ].sort_values("amount", ascending=False)
            unusual_transactions = unusual_transactions.head(5)
            unusual_list = unusual_transactions[
