...
def connect_bank():
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not access_token:
                flash(
                    "Failed to connect to bank. Please check your credentials.",
                    "danger",
                )
            ...
            accounts = BankAccount.query.filter_by(
                user_id=current_user.id, bank_name=bank_name
            ).all()

            transaction_count = 0
            for account in accounts:
                transactions = fetch_transactions(
                    bank_name, access_token, account.account_number
                )
            ...
            flash(
                f"Successfully connected to {bank_name} and imported {transaction_count} new transactions.",
                "success",
            )
...
def sync_bank_account(account_id):
    ...
    try:
        ...
        new_balance, transaction_count = sync_account_data(
            account.bank_name, account.access_token, account.account_number, account.id
        )
        ...
        flash(
            f"Successfully synced account. Found {transaction_count} new transactions.",
            "success",
        )
    except Exception as e:
    ...
    if not recommendations:
        try:
            ...
            if os.environ.get("DEEPSEEK_API_KEY"):
                ...
                logger.info(
                    f"Generated {count} AI recommendations, using_ai={using_ai}"
                )
            else:
            ...
            flash(
                "Произошла ошибка при создании рекомендаций. Пожалуйста, попробуйте позже.",
                "danger",
            )

    return render_template(
        "recommendations.html", recommendations=recommendations, using_ai=using_ai
    )
...
def generate_new_recommendations():
    try:
        ...
        if os.environ.get("DEEPSEEK_API_KEY"):
            ...
            if using_ai:
                flash(
                    f"Сгенерировано {count} новых ИИ-рекомендаций для вас.", "success"
                )
            else:
                flash(
                    f"Сгенерировано {count} новых рекомендаций для вас. ИИ временно недоступен, использованы стандартные рекомендации.",
                    "info",
                )
        else:
            # Fall back to rule-based recommendations
