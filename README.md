...
Для запуска с использованием pytest напрямую:
```bash
# Запуск всех тестов
python -m pytest
...
python -m pytest -m models      # Тесты моделей данных
python -m pytest -m routes      # Тесты маршрутов
python -m pytest -m security    # Тесты безопасности
python -m pytest -m services    # Тесты сервисов

# Запуск конкретного теста
python -m pytest tests/test_models.py::test_password_hashing
```

Тесты организованы с использованием фреймворка pytest, что делает их более читаемыми и поддерживаемыми. Для каждой категории тестов добавлены маркеры, позволяющие запускать подмножества тестов по необходимости.
